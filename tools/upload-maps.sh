...
while read -r clanNo handle; do
  for turn in 0899-12; do
    wxxFile=${clanNo}/data/output/${turn}.${clanNo}.wxx
    wxxUploadedFile=${clanNo}/data/logs/${turn}.${clanNo}.wxx.uploaded
    [ -f "${wxxFile}" ] || continue
    [ -f "${wxxUploadedFile}" ] && {
      [ "${wxxUploadedFile}" -nt "${wxxFile}" ] && {
        echo "${wxxFile}: skipping (uploaded)"
        continue
      }
      rm -f ${wxxUploadedFile}
    }
    echo "${wxxFile}: uploading..."
    bin/ottoapp -N --db ${OTTOAPP_DBPATH} game upload --game 0301 --handle ${handle} ${wxxFile} --can-read --can-delete --can-share=true --can-write=false || {
      echo " error: clan ${clanNo}: handle ${handle}"
      continue
    }
    touch ${wxxUploadedFile}
    echo "${wxxFile}: uploaded"
  done
done

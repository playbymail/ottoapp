...
jq -r 'to_entries | sort_by(.key) | .[] | "\(.key) \(.value)"' clans.json |
while read -r clanNo handle; do
  # echo "upload: clan ${clanNo}: handle ${handle}"
  for turn in 0899-12; do
    reportFile=${clanNo}/data/input/${turn}.${clanNo}.report.docx
    reportUploadedFile=${clanNo}/data/logs/${turn}.${clanNo}.report.docx.uploaded
    [ -f "${reportFile}" ] || continue
    [ -f "${reportUploadedFile}" ] && {
      [ "${reportUploadedFile}" -nt "${reportFile}" ] && {
        echo "${reportFile}: skipping (uploaded)"
        continue
      }
      rm -f ${reportUploadedFile}
    }
    echo "${reportFile}: uploading..."
    bin/ottoapp -N --db ${OTTOAPP_DBPATH} game upload --game 0301 --handle ${handle} ${reportFile} --can-read --can-delete=false --can-share=true --can-write=false || {
      echo " error: clan ${clanNo}: handle ${handle}"
      continue
    }
    touch ${reportUploadedFile}
    echo "${reportFile}: uploaded"
  done
done

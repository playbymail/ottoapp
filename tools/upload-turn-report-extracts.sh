...
while read -r clanNo handle; do
  for turn in 0899-12; do
    reportExtractFile=${clanNo}/data/input/${turn}.${clanNo}.report.txt
    reportExtractUploadedFile=${clanNo}/data/logs/${turn}.${clanNo}.report.txt.uploaded
    [ -f "${reportExtractFile}" ] || continue
    [ -f "${reportExtractUploadedFile}" ] && {
      [ "${reportExtractUploadedFile}" -nt "${reportExtractFile}" ] && {
        echo "${reportExtractFile}: skipping (uploaded)"
        continue
      }
      rm -f ${reportExtractUploadedFile}
    }
    echo "${reportExtractFile}: uploading..."
    bin/ottoapp -N --db ${OTTOAPP_DBPATH} game upload --game 0301 --handle ${handle} ${reportExtractFile} --can-read --can-delete=false --can-share --can-write=false || {
      echo " error: clan ${clanNo}: handle ${handle}"
      continue
    }
    touch ${reportExtractUploadedFile}
    echo "${reportExtractFile}: uploaded"
  done
done

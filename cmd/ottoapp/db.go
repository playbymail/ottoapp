...
func cmdDb() *cobra.Command {
	...
	cmd.AddCommand(cmdDbClone())
	cmd.AddCommand(cmdDbCompact())
	cmd.AddCommand(cmdDbCreate())
	cmd.AddCommand(cmdDbInit())
	cmd.AddCommand(cmdDbMigrate())
	//cmd.AddCommand(cmdDbSeed())
	cmd.AddCommand(cmdDbVersion())
	err := addFlags(cmd)
	if err != nil {
		log.Fatalf("db: %v\n", err)
	}
...
func cmdDbBackup() *cobra.Command {
	outputPath := ""
	addFlags := func(cmd *cobra.Command) error {
		cmd.Flags().StringVar(&outputPath, "output", outputPath, "path to directory for backup file (must exist)")
...
func cmdDbInit() *cobra.Command {
	overwrite := false
	addFlags := func(cmd *cobra.Command) error {
		cmd.Flags().BoolVar(&overwrite, "overwrite", overwrite, "overwrite existing database")
		return nil
	}
	cmd := &cobra.Command{
		Use:          "init <path-to-database>",
...
func cmdDbMigrate() *cobra.Command {
	...
	cmd.AddCommand(cmdDbMigrateStatus())
	cmd.AddCommand(cmdDbMigrateUp())
	err := addFlags(cmd)
	if err != nil {
...
func cmdDbMigrateUp() *cobra.Command {
...
func cmdDbMigrateStatus() *cobra.Command {
	const checkVersion, isInitializing = false, false
	addFlags := func(cmd *cobra.Command) error {
		return nil
	}
...
func cmdDbVersion() *cobra.Command {
	addFlags := func(cmd *cobra.Command) error {
		return nil
	}
	cmd := &cobra.Command{

...
export default class Profile extends Component {
  ...
  // ---- convenience booleans for strict templates (avoid truth-helpers) ----
  get hasChanges() {
    return (
  ...
  // optional: useful while wiring
  get dumpPermissions() {
    // NOTE: uses local permissions taken from the model; remove if noisy
  ...
  @action async save(e) {
    ...
    this.email = this.args.model?.email ?? this.email;
    this.timezone = this.args.model?.timezone ?? this.timezone;
  }
  ...
  <template>
    <form {{on "submit" this.save}} class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {{#if this.args.errorMessage.length}}
          ...
          <div class="flex">
            ...
            <div class="ml-3">
              ...
              <ul role="list" class="mt-2 list-disc space-y-1 pl-5 text-sm text-red-700">
                {{#each this.args.errorMessage as |msg|}}
                  <li>{{msg}}</li>
                {{/each}}
              </ul>
            </div>
      ...
      <div class="mt-6 flex items-center justify-end gap-x-6">
        ...
        >
          {{if this.args.isSaving "Saving..." "Save"}}
        </button>
      </div>
    </form>
  </template>

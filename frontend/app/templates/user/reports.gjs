    ...
    <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <header class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-4">
          <LinkTo @route="user.dashboard" class="text-gray-500 hover:text-gray-700 xl:hidden">
      ...
      <section class="flex h-full flex-col rounded-lg bg-white p-6 shadow-sm ring-1 ring-gray-200">
        {{#if @model.length}}
          <ul class="flex-1 space-y-3">
            {{#each @model as |file|}}
              <li class="flex items-center justify-between gap-4 rounded-md border border-gray-100 bg-gray-50 px-3 py-2 text-sm">
                <div class="min-w-0">
